...
This system supports live trading on the Binance exchange. When enabled, the trading signals generated by your configured strategies can be automatically executed as market orders.

### 1. Enabling Live Trading

To activate live trading, you need to set an environment variable:
*   In your `.env` file, set `LIVE_TRADING_ENABLED=True`.
    *   If this variable is `False` (default) or not set, the system will not execute trades, even if `mode: live` is set in `config.yaml`.

### 2. API Key Configuration

Securely configure your Binance API keys for trading:
*   Set `BINANCE_API_KEY` and `BINANCE_API_SECRET` in your `.env` file.
    *   `BINANCE_API_KEY="YOUR_ACTUAL_BINANCE_API_KEY"`
    *   `BINANCE_API_SECRET="YOUR_ACTUAL_BINANCE_API_SECRET"`
*   These keys must be generated from your Binance account. Ensure they have the necessary permissions for spot trading. Keep these keys confidential.

### 3. Signal Processing and Order Execution

When live trading is enabled:
*   The system uses the final decisions from the portfolio management node (which are based on your strategy signals).
*   For each asset, if a decision includes an `action` ("buy" or "sell") and a `quantity`, the system will attempt to place a **market order** on Binance for the specified quantity.
*   The `place_order_from_signal` function in `src.gateway.binance.execution.py` handles the quantity adjustments based on Binance's symbol filters (e.g., `LOT_SIZE`, `MIN_NOTIONAL`) and places the order.
*   Order execution results and any errors are logged and returned by the `Agent.run()` method.

### 4. Important Disclaimer

**Disclaimer:** Live automated trading carries significant financial risk. Ensure you understand the strategy and the system's behavior before enabling live trading. Use at your own risk. It is recommended to start with small amounts or use a testnet account if available and supported by this integration (note: current integration targets live spot market). The developers of this project are not responsible for any financial losses incurred.
